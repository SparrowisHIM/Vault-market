...
export function ResearchTicker({ items }: ResearchTickerProps) {
  ...
  return (
    <div className="research-ticker border-t border-white/10 bg-[rgba(12,14,11,0.68)]">
      ...
      <div className="research-ticker-track flex items-center gap-4 py-2.5 pl-4 font-mono text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-vault-paper/58">
        {loop.map((item, index) => (
          <span
            key={`${item}-${index}`}
            className="shrink-0 rounded-full border border-white/10 bg-white/[0.045] px-3 py-1 text-vault-paper/72"
          >
            {item}
          </span>
        ))}
