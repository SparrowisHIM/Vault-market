...
export function CompareDrawer({
  ...
  const compareGridClass = cn(
    "grid gap-4",
    listings.length === 1 &&
      "mx-auto w-full max-w-[32rem] grid-cols-1",
    listings.length === 2 &&
      "mx-auto w-full max-w-6xl grid-cols-1 lg:grid-cols-2",
    listings.length >= 3 &&
      "mx-auto w-full max-w-7xl grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3",
  );
      ...
      {renderDrawer ? (
        <div className="fixed inset-0 z-50">
          <button
            ...
            onClick={onClose}
            className="absolute inset-0 bg-[rgba(17,19,15,0.68)]"
          />
          <section
            ref={panelRef}
            role="dialog"
            ...
            className="absolute inset-0 flex min-h-dvh flex-col overflow-hidden border border-[rgba(255,255,255,0.08)] bg-[var(--surface-canvas)] shadow-[0_0_90px_rgba(17,19,15,0.3)] pb-[env(safe-area-inset-bottom,0px)]"
          >
            <header className="shrink-0 border-b border-[var(--border-soft)] bg-[rgba(248,247,243,0.96)] px-4 py-4 shadow-[0_12px_38px_rgba(17,19,15,0.06)]">
              <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0">
                  <p className="font-mono text-[0.66rem] font-semibold uppercase tracking-[0.16em] text-vault-registry">
                    Inspection comparison
                  </p>
                  <h2 id="compare-drawer-title" className="mt-1 text-2xl font-semibold leading-tight text-vault-ink">
                    Compare queued slabs
                  </h2>
                  <p className="mt-1 max-w-2xl text-sm leading-6 text-vault-steel">
                    Review grade, ask, custody, comps, and seller trust across up to 3 slabs.
                  </p>
                </div>
                <div className="flex items-center justify-between gap-3 lg:justify-end">
                  <div className="hidden rounded-[8px] border border-[var(--border-soft)] bg-white/56 px-3 py-2 lg:block">
                    <p className="font-mono text-[0.58rem] font-semibold uppercase tracking-[0.14em] text-vault-steel">
                      Desk capacity
                    </p>
                    <p className="mt-0.5 text-sm font-semibold text-vault-ink">
                      {listings.length} of 3 selected
                    </p>
                  </div>
                  <button
                    ref={closeButtonRef}
                    type="button"
                    onClick={onClose}
                    className="grid h-10 w-10 shrink-0 place-items-center rounded-[7px] border border-[var(--border-soft)] bg-white/64 text-vault-graphite transition hover:bg-white/92 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]"
                    aria-label="Close compare desk"
                  >
            ...
            <div
              ref={bodyRef}
              className="flex-1 overflow-y-auto bg-[linear-gradient(180deg,rgba(235,230,220,0.74),var(--surface-canvas)_18rem)] px-4 py-5 sm:px-6 lg:px-8"
            >
              {listings.length > 0 ? (
                <div className={compareGridClass}>
                  {listings.map((listing) => (
                    <article
                      key={listing.id}
                      className="compare-column min-w-0 rounded-[12px] border border-[var(--border-soft)] bg-[rgba(248,247,243,0.96)] p-3 shadow-[0_18px_52px_rgba(17,19,15,0.1)]"
                    >
                      <div className="rounded-[9px] border border-[var(--border-soft)] bg-white/54 p-3">
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          <span className="rounded-[6px] border border-[var(--border-soft)] bg-white/58 px-2.5 py-1 font-mono text-[0.62rem] font-semibold uppercase tracking-[0.12em] text-vault-graphite">
                            {listing.gradingCompany} {listing.grade}
                          </span>
                          <span className="rounded-[6px] border border-[var(--border-soft)] bg-white/58 px-2.5 py-1 font-mono text-[0.62rem] font-semibold uppercase tracking-[0.12em] text-vault-steel">
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                      ...
                      <Link
                        href={buildListingHref(listing.slug, sourceHref)}
                        className="mt-1 inline-flex h-10 items-center justify-center gap-2 rounded-[7px] border border-vault-graphite bg-vault-ink px-4 text-sm font-semibold text-vault-paper transition hover:bg-vault-graphite focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] sm:col-span-2 lg:col-span-1 xl:col-span-2"
                      >
                        Inspect slab
                        <ArrowRight className="h-4 w-4" aria-hidden="true" />
                      </Link>
            ...
            <footer className="shrink-0 border-t border-[var(--border-soft)] bg-[rgba(248,247,243,0.96)] p-4">
